...
def mro(bases_dict: Dict, cls: str):
    ...
    if not bases_dict[cls]:
        ...
        return result + _merge(
            *[
                mro(bases_dict, kls)
                for kls in bases_dict[cls]
                if kls in all_class_names
            ],
            bases_dict[cls],
        )
...
class BaseContexts:
    ...
    def get_function_name(self, node, program_content):
        ...
        if node.type == self.FUNCTION_DEFINITION:
            func_name = bytes(program_content, "utf8")[
    ...
    def get_class_name(self, node, program_content):
        ...
        if node.type == self.CLASS_DEFINITION:
            class_name = bytes(program_content, "utf8")[
                node.children[1].start_byte : node.children[1].end_byte
            ].decode("utf8")
        else:
            for child in node.children:
    ...
    def is_class_node(self, node):
        ...
        # class with single/multiple decorator
            ...
            node.type == self.DECORATED_DEFINITION
    ...
    def is_function_node(self, node):
        ...
        if (
            node.type == self.FUNCTION_DEFINITION
    ...
    def get_invariant_enclosed_functions(self, root_node):
        ...
        for node in root_node.children:
            if self.is_function_node(node):
                ...
                node.type in self.BLOCK_INVARIANT_TYPES
    ...
    def get_class_bases(self, block_header: str):
        ...
        if "(" in block_header and "):" in block_header:
            parent_classes = (
                block_header.split("(")[-1].strip().split("):")[0].strip()
            ).split(",")
    ...
    def get_class_MRO(self, program_content: str, cls: str):
        ...
        for class_block in all_classes:
            ...
            class_bases_dict[class_name] = self.get_class_bases(
                class_block.block_header
            )

        try:
            class_mro = mro(class_bases_dict, cls)
    ...
    def get_all_functions(
        ...
        parent: str,
        depth: int,
        block_header: str = DEFAULT_BLOCK_HEADER,
        class_MRO: Tuple[str, ...] = ("__", "__class__"),
    ):
        """
        ...
        for i in range(len(children)):
            ...
            # children[i] is of BLOCK_INVARIANT_TYPES
                ...
                children[i].type in self.BLOCK_INVARIANT_TYPES
                or children[i].type == self.BLOCK_DEFINITION
            ):
                invariant_enclosed_function_nodes = (
                    self.get_invariant_enclosed_functions(children[i])
                )
                invariant_enclosed_class_nodes = self.get_invariant_enclosed_classes(
                    children[i]
                )
                ...
                for class_node in invariant_enclosed_class_nodes:
                    ...
                    temp_block_header = self.get_block_header(
                        program_content, class_node
                    )
                    temp_class_MRO = self.get_class_MRO(program_content, class_name)
                    current_class_MRO = class_MRO + temp_class_MRO
    ...
    def get_all_classes(self, program_content: str):
        ...
        for i in range(len(children)):
            ...
            if self.is_class_node(children[i]) is not None:
                ...
                current_class_MRO = def_class_MRO + tuple(
                    self.get_class_bases(block_header)
                )
    ...
    def get_all_blocks(self, program_content: str, content_type=None):
        ...
        left_out_lines = [
            i for i in range(root_node.end_point[0] + 1) if i not in block_lines
        ]
        ...
        for class_block in all_classes:
            ...
            class_specific_block_content = "\n".join(
                lines[j] for j in class_specific_block_lines
            )
    ...
    def preprocess_import(self, statement):
        ...
        statement = statement.replace("\n", "")
        module_group = statement.split("as ")[0].strip().split("import ")[1].strip()
    ...
    def get_import_block(self, program_content):
        ...
        for import_line in self.IMPORT_STATEMENTS:
            ...
            if import_line_start == import_line_end:
                ...
                import_line_indices.extend(
                    [i for i in range(import_line_start, import_line_end + 1)]
                )

        import_block = Block(
            -1, -1, import_line_indices, "\n".join(import_content), "", IMPORT
        )
    ...
    def get_module(self, program_content: str):
        ...
        module_body = bytes(program_content, "utf8")[
        ...
        module_block = Block(
            module_start, module_end, [], module_body, "", ROOT_BLOCK_TYPE
        )
        ...
        for block in all_blocks:
            if start_line >= block.start_line and end_line <= block.end_line:
                if len(block.other_lines) == 0:
                    ...
                    if (
                        start_line in block.other_lines
    ...
    def get_transitive_parent_classes(self, parent_class_names, all_class_blocks, k):
        ...
        for c_block in all_class_blocks:
            if (c_block.metadata.split(".")[-1] in parent_class_names) and (
                len(c_block.mro) > 2
            ):
        ...
        if len(all_parent_class_names) == k:
            ...
            return self.get_transitive_parent_classes(
                all_parent_class_names, all_class_blocks, len(all_parent_class_names)
            )
    ...
    def get_local_and_super_class(self, program_content, start_line, end_line):
        ...
        if local_class_block is not None:
            ...
            parent_class_names = self.get_transitive_parent_classes(
                parent_class_names, class_blocks, len(parent_class_names)
            )
    ...
    def get_transitive_child_classes(self, child_class_names, all_class_blocks, k):
        ...
        if len(all_child_class_names) == k:
            ...
            return self.get_transitive_child_classes(
                all_child_class_names, all_class_blocks, len(all_child_class_names)
            )
    ...
    def get_local_and_child_class(self, program_content, start_line, end_line):
        ...
        if local_class_block is not None:
            ...
            child_class_names = self.get_transitive_child_classes(
                child_class_names, class_blocks, len(child_class_names)
            )
                    ...
                    if len(matches) == 0:
                        ...
                        for match in matches:
                            ...
                            if "super" in match.strip():
                                temp_possible_super_class = self.get_class_MRO(
                                    program_content, temp_super_class
                                )[0]
                            else:
        ...
        for cls in class_mro:
            if cls in possible_block_classes:
                for block in possible_blocks:
                    block_func_name = block.metadata.split(".")[-1]
                    block_class_name = block.metadata.split(".")[-2]
                    if block_func_name == func_name and block_class_name == cls:
                        return block
